...
3. **无法确定**：上述两者都缺失 → 在回复中询问用户"请指定本次会话的角色：initer / planner / implementer / reviewer"，等待用户确认后再继续

**确定角色后**：
- 读取对应的 `agents/<role>/SKILL.md`
- 按 SKILL.md 中定义的输入顺序和工作流程执行
- 不要跳过任何输入读取步骤

**角色切换规则**：
- 除用户显式要求外，不允许在同一次会话中切换角色
- 一个 agent 完成交付/交接后，会话就此结束
- 下一个 agent 必须在新的独立会话中启动
